...
const toLastNames = people => {
  ...
  let newArr=[];
  people.map(i=>{
    newArr.push(`${i.firstName} ${i.lastName}`);
  });
  return newArr;
};
...
addPurchases([{item: 'switch', purchasePrice: 399}, {item: 'toothpaste', purchasePrice: 2}])).toStrictEqual(401);
------------------------------------------------------------------------------------------------ */
...
const returnNames = (arr) => {
  ...
  arr.reduce((acc,value)=>{
    return newArr1.push(value.name);
  },-1);
  return newArr1;
};
