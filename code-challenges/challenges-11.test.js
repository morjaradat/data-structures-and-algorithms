...
const count = (target, input) => {
  ...
  let count = 0;
  input.map(i => {
    i.filter(num => num === target ? count++ : 0);
  });
  return count;
};
...
const totalSum = (input) => {
  ...
  for (var a = 0; a < input.length; a++) {
    typeof input[a] === 'number' ? sum += input[a] : input[a] instanceof Array? sum += totalSum(input[a]) :NaN ;
    // if (typeof input[a] === 'number') {
    //   sum += input[a];
    // } else if (input[a] instanceof Array) {
    //   sum += totalSum(input[a]);
    // }
  }
  return sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(num => num.filter(x => x === Number(x) && x % 5 === 0).map(x=>Math.pow(2,x)));
};
...
let findShortest = (data) => {
  ...
  let name= data[0];
  data.map((value)=>{
    if(parseInt(value.height) < parseInt(name.height)){
      name = value;
    }
  });
  return name.name;
